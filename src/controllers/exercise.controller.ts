...
export const  create = async (req: Request, res: Response, next: NextFunction) => {
    try { 
        ...
        const {name, imgUrl, videoUrl, duration, benefits, level, description } = result.data;
        const exercise = await exerciseService.createExercise({name, imgUrl, videoUrl, duration, benefits, level, description });
...
export const update = async  (req: Request<{id: string}>, res: Response, next: NextFunction) => {
    try{
        ...
        const {name, imgUrl, videoUrl, duration, benefits, level, description  } = result.data;
        const exercise = await exerciseService.updateExercise({name, imgUrl, videoUrl, duration, benefits, level, description }, req.params.id )
        res.status(200).json(exercise)
    } catch(error){
        next(error)
    }
